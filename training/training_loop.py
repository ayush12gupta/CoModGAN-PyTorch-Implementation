    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for round_idx, (real_img, real_c, gen_z, gen_c, mask, ldmks, txtr_img, realtxtr_img) in enumerate(zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c, phase_masks, phase_ldmks, phase_txtr_img, phase_realtxtr_img)):
                ...
                dgenLoss += loss_dgen.cpu()
                symLoss += loss_sym.cpu()
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images = torch.cat([G_ema(z=z, c=c, image_in=img, mask_in=mask, noise_mode='const').cpu() for z, c, img, mask in zip(grid_z, grid_c, grid_txtr_images, grid_mask_images)]).numpy()
