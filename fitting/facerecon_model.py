...
from fitting.util import util 
from fitting.util.nvdiffrast import MeshRenderer
from fitting.util.preprocess import estimate_norm_torch
...
class FaceReconModel(BaseModel):
    ...
    #         parser.add_argument('--w_exp', type=float, default=4.0, help='weight for exp_reg loss')
    #         parser.add_argument('--w_tex', type=float, default=1.7e-2, help='weight for tex_reg loss')
    #         parser.add_argument('--w_gamma', type=float, default=20.0, help='weight for gamma loss')
    ...
    def __init__(self, savedir, model_dir, device, pretrained_name=None, checkpoints_dir='/checkpoints'):
        ...
        """
        checkpoints_dir = model_dir + checkpoints_dir
        BaseModel.__init__(self, savedir, checkpoints_dir, pretrained_name, device)  # call the initialization method of BaseModel
        ...
        #         net_recog='r50', pretrained_path=net_recog_path
        #         )
    ...
    def forward(self, input_img):
        ...
        # face_color_tv = TexturesVertex(self.pred_color)
        # meshes = Meshes(self.pred_shape, self.facemodel.face_buf.repeat(n_b, 1, 1), face_color_tv)
        # rendered_img = self.renderer(mesh)
        # self.pred_mask, _, self.pred_face = self.renderer(
        #     self.pred_vertex, self.facemodel.face_buf, feat=self.pred_color)
        self.pred_coeffs_dict = self.facemodel.split_coeff(output_coeff)

        return self.pred_shape
